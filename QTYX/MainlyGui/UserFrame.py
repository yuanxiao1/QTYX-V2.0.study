...
from MainlyGui.ElementGui.DefDialog import (
    ...
    WebDialog, MarketGraphDialog, BacktGraphDialog
)
...
class UserFrame(wx.Frame):
    ...
    def add_patten_para_lay(self, sub_panel):
        ...
        # 选股按钮
        self.pick_patten_but = wx.Button(sub_panel, -1, "开始回测")
    ...
    def _download_st_data(self): # 复位选股按钮事件
        ...
        select_src = ChoiceDialog(u"选股数据源选取", [u"tushare每日指标",
                                                    u"爬虫接口获取",
                                                    u"基金季度持仓",
                                                    u"离线每日指标",
                                                    u"离线业绩预告",
        ...
        elif select_src == "爬虫接口获取":

            select_msg = ChoiceDialog(u"爬虫数据类型选择", list(self.crawer_backend.keys()))

            if (select_msg == "爬虫每日涨停") or (select_msg == "爬虫北向资金") :
                MessageDialog("2.0.study 未开放该功能！")
                return
    ...
    def _ev_funt_guide_menu(self, event):
        webbrowser.open('https://zhuanlan.zhihu.com/p/467160411')
