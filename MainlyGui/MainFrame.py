...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent=None, id=-1, displaySize=(1600, 900), Fun_SwFrame=None):
        ...
        toolbar.AddTool(1101, '', wx.Bitmap(MainFrame.rel_path + "png/tab_trade.png"))
        toolbar.AddSeparator()
        toolbar.AddTool(1102, '', wx.Bitmap(MainFrame.rel_path + "png/tab_config.png"))
        toolbar.Realize()
    ...
    def OnEventTrig(self, event):
        if event.GetId() == 1100:  # 量化按钮
            self.fun_swframe(1)
        elif event.GetId() == 1101:  # 交易按钮
            self.fun_swframe(2)
        elif event.GetId() == 1102:  # 配置按钮
            self.fun_swframe(3)
        else:
            pass
